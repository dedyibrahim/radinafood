  ...
  <div class="auth-wrapper auth-v1 background-login">
    <div class="auth-inner">
      ...
      <v-btn variant="outlined" color="grey" block class="text-capitalize">
        <icon size="25" class="mr-3" name="flat-color-icons:google"></icon> Sign
        With Google
      </v-btn>
      <div class="text-center">
        <p class="font-weight my-5 background-text">Sign In with your Email</p>
      ...
      <v-form ref="form">
        ...
        <div
          class="text-subtitle-1 mt-0text-medium-emphasis d-flex align-center justify-space-between"
        >
          <v-checkbox label="Keep Me Logged in" hide-details=""></v-checkbox>

          <a
            class="text-caption text-decoration-none text-blue font-weight-bold"
            href="#"
      ...
      <div
        class="text-subtitle-1 mt-2 text-medium-emphasis d-flex align-center"
      >
        Don't Have Account ?
        <NuxtLink
          to="/SignUp"
    ...
    async login() {
      ...
      try {
        ...
        useStore().$patch({
          ...
          snackbar: true,
          messageSnack: "Your Login Successfully",
        });

     
        await navigateTo({ path: '/Otp', query:data })
       } catch (error) {
        useStore().$patch({
          snackStatus: "red",
          snackbar: true,
